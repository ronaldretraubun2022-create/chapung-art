<article class="group overflow-hidden rounded-lg border border-zinc-800 bg-zinc-950 shadow-xl shadow-black/20 transition hover:border-yellow-600/70">
    <a href="{{ route($showRouteName ?? 'news.show', $post->slug) }}" class="block">
        @include('partials.public.image', ['path' => $post->display_image, 'alt' => $post->title, 'ratio' => 'aspect-[16/10]', 'label' => __('chapung.types.news')])
        <div class="p-5">
            <div class="flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.16em] text-zinc-500">
                <span>{{ $post->category?->name ?: __('chapung.types.news') }}</span>
                <span class="h-1 w-1 rounded-full bg-yellow-600"></span>
                <time>{{ optional($post->published_at ?: $post->created_at)->format('d M Y') }}</time>
                @if ($post->reading_time)
                    <span class="h-1 w-1 rounded-full bg-yellow-600"></span>
                    <span>{{ __('chapung.pages.news.minutes_read', ['count' => $post->reading_time]) }}</span>
                @endif
            </div>
            <h3 class="mt-4 line-clamp-2 text-xl font-black uppercase tracking-tight text-white">{{ $post->title }}</h3>
            <p class="mt-2 text-xs font-bold uppercase tracking-[0.16em] text-zinc-500">{{ $post->author?->name ?: $post->author_name ?: __('chapung.pages.news.author_fallback') }}</p>
            <p class="mt-3 line-clamp-3 text-sm leading-6 text-zinc-400">{{ $post->excerpt ?: str(strip_tags($post->content))->limit(150) }}</p>
            <span class="mt-5 inline-flex border-b border-yellow-600 pb-1 text-xs font-black uppercase tracking-[0.2em] text-yellow-500">{{ __('chapung.pages.news.read') }}</span>
        </div>
    </a>
</article>
