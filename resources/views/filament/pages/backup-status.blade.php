    ...
    <div class="space-y-6">
        <div class="grid gap-4 md:grid-cols-3">
            <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <p class="text-xs font-medium uppercase text-gray-500">Backup Files</p>
                <p class="mt-2 text-2xl font-semibold text-gray-950 dark:text-white">{{ number_format(count($files)) }}</p>
            </div>

            <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <p class="text-xs font-medium uppercase text-gray-500">Latest Backup</p>
                <p class="mt-2 text-lg font-semibold text-gray-950 dark:text-white">{{ $this->getLatestBackupLabel() }}</p>
            </div>

            <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <p class="text-xs font-medium uppercase text-gray-500">Total Size</p>
        ...
        <section class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <h2 class="text-base font-semibold text-gray-950 dark:text-white">Private Storage</h2>
            <p class="mt-2 break-all text-sm text-gray-600 dark:text-gray-300">{{ $this->getBackupDirectoryPath() }}</p>
            <p class="mt-2 text-sm text-gray-500">Backup metadata only. File download is intentionally disabled from admin UI.</p>
        </section>

        <section class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <h2 class="text-base font-semibold text-gray-950 dark:text-white">Backup History</h2>
                ...
                <table class="w-full text-left text-sm">
                    <thead class="border-b border-gray-200 text-xs uppercase text-gray-500 dark:border-gray-800">
                        <tr>
                            ...
                            <th class="py-3 pr-4">Stored Path</th>
                            <th class="py-3 pr-4 text-right">Size</th>
                            <th class="py-3 pr-4">Modified</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="py-6 text-center text-gray-500">Belum ada backup. Jalankan command backup atau klik Run Backup.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
