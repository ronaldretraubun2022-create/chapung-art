...
@section('seo')
    @include('partials.seo-meta', ['seo' => seo_meta('collections.show', fallback: [
        'title' => $collection->name.' '.__('chapung.types.collection').' | '.site_setting('site_name', 'Chapung Art'),
        'description' => (string) str(strip_tags($collection->description ?: __('chapung.home.collection_fallback')))->limit(160),
...
@section('content')
    ...
    <section class="px-4 py-14 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-7xl space-y-14">
            <div>
                <h2 class="text-2xl font-black uppercase tracking-tight text-white">{{ __('chapung.types.artwork') }}</h2>
                @if ($artworks->count())
            ...
            <div>
                <h2 class="text-2xl font-black uppercase tracking-tight text-white">{{ __('chapung.types.photography') }}</h2>
